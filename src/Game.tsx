...
function Game({ winStreak: initialWinStreak, updateWinStreak, maxGuesses, hidden, difficulty, colorBlind, keyboardLayout, noKeyGrab }: GameProps) {
  ...
  const onKey: (key: string) => void = useCallback((key: string) => {
    ...
    if (/^[a-z]$/i.test(key)) {
      ...
      for (const g of guesses) {
        ...
        const feedback = violation(difficulty, c, currentGuess);
        if (feedback) {
          setHint(feedback);
          return;
        }
  ...
  return (
    <div className="Game" style={{ display: hidden ? "none" : "block" }}>
      <div className="Game-options">
        <label htmlFor="wordLength">Letters ({wordLength}):</label>
        <input
          type="range"
          min={minLength}
          max={maxLength}
      ...
      <div className="extras">
          ...
          <p>made by <a href="https://github.com/profcoder69420">profcoder69420 on GitHub</a> and <a href="https://github.com/AnhNguyenlost13">AnhNguyenLost13 on GitHub</a></p>
      </div>  
    </div>
  );
}
